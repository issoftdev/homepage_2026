...
* **우측 CI 및 공식 인증 배치 비율 조율**: 우측 CI 영역을 모던하게 배치하고 가시성을 확보했습니다.
* **반응형 조직도(Org Tree)**: 이미지 대신 반응형 HTML/CSS 계통 조직도로 완전히 새로 구성하여 모바일에서도 깨지지 않는 깨끗한 텍스트 가시성을 확보했습니다.
* **인증서 전시 UI 전면 리뉴얼**: 상단 VISION의 3단 카드 그리드 디자인과의 시각적 중복을 피하기 위해, 실제 업로드된 인증서 스캔본 이미지를 표출하는 **인터랙티브 쇼케이스 갤러리(Interactive Showcase Gallery) UI**를 새롭게 설계 및 도입했습니다. 우측의 리스트에서 특정 인증서(ISO 9001, Main-Biz, 벤처기업, 연구개발부서, L-CRIS 저작권)를 클릭하면 좌측의 고해상도 액티브 프레임 공간 내에 실물 인증서 원본이 트랜지션 애니메이션과 함께 교체 표출되어 회사의 공인 기술 경쟁력을 세련되게 입증합니다.
...
* **가로형 스티키 메뉴바 및 순차 스크롤 레이아웃**: 기존의 탭 클릭 전환식(Zero-JS 라디오 기법) 구조를 탈피하고, **상단 고정형 가로 스티키 메뉴바와 전체 본문 순차 스택(Scroll & Stack) 레이아웃**으로 변경했습니다. 사용자가 스크롤을 내릴 때 화면에 표시되는 영역을 자동으로 감지하여 메뉴바의 활성화 상태를 실시간 스위칭하는 **스크롤 스파이(IntersectionObserver)** 기능을 도입하고, 부드러운 스무스 앵커 스크롤과 헤더 겹침 보정(`scroll-margin-top`)을 적용하여 콘텐츠의 유기적 접근성을 배가했습니다.

### 4) 구축사례(Portfolio) 및 문의하기(Contact)
* **'CASE P16' 텍스트 완전 제거**: 구축사례 카드 우측 상단의 불필요한 일련번호 텍스트를 전면 걷어내어 심플하게 구성했습니다.
* **구축사례(Portfolio) 정렬 및 파트너사 로고 동기화**:
...
* **문의하기(Contact) 페이지 UI 전면 개편**:
  - **Emojis 제거 및 인라인 SVG 벡터 아이콘 도입**: 예전 느낌이 풍기던 투박한 이모지들을 제거하고 오렌지색 베젤이 적용된 프리미엄 바닐라 SVG 아이콘들로 대체했습니다.
  - **연락처 3단 카드 레이아웃화**: 세로형 단순 나열 레이아웃을 3분할 가로 카드로 구성하여 시각적 가치와 가독성을 높였습니다.
  - **온라인 도입 문의 폼(Contact Form) 탑재 및 Web3Forms 실시간 메일 연동**: B2B 고객의 접근성을 강화하기 위해 온라인 문의 신청 폼을 신설하고, AJAX Fetch 비동기 통신을 통해 **Web3Forms API(Access Key: `e76173f8-6668-4b7e-b384-388e6ca3c0d8`)를 연동하여 `ckkang@issoftware.co.kr`로 실시간 메일 전송 및 성공 피드백 오버레이**가 동작하도록 시스템을 구축했습니다.
...
### 3) 실무 배포 절차 및 명령어 순서 (일상적인 작업 흐름)
로컬에서 수정한 소스 코드를 서버에 무결하게 배포할 때 사용하는 커맨드 순서입니다. 로컬에서 직접 빌드 결과물(`dist/`)을 업로드하실 필요 없이, 깃허브 푸시만 하시면 원격 서버가 자동 빌드 및 배포를 완료합니다.

```bash
# 1단계: (권장) 로컬 사전 빌드 검사로 컴파일 에러 유무 진단
cd issoft-renewal
npm run build
cd ..

# 2단계: 로컬 Git 저장소에 소스 수정 내역 스테이징 및 커밋
git add .
git commit -m "수정 내역에 대한 설명 메시지 작성"

# 3단계: 원격 저장소 푸시 (이 명령어로 배포 액션이 자동 실행됨)
