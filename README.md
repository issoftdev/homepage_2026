...
### 2) 네이버 서치어드바이저 및 구글 서치콘솔 신규 등록 (완료)
* **현황**: 네이버 서치어드바이저(소유권 인증 메타태그 `naver-site-verification` 적용) 및 구글 서치콘솔에 `https://issoft.kr` 사이트 소유권 등록과 사이트맵(`sitemap-index.xml` / `sitemap-0.xml`) 제출이 성공적으로 완료되었습니다.
* **효과**: 신규 도메인과 리뉴얼된 모든 국/영문 페이지군에 대해 검색 로봇의 실시간 수집 및 색인화가 개시되어 네이버/구글 검색 노출 가시성이 점진적으로 상승하게 됩니다.
